...
pub fn trim_to_boundary(s: &str, max_len: usize) -> &str {
    ...
    // If next char is a space, we're at a word boundary
    if s.as_bytes()[max_len] == b' ' {
        return s[..max_len].trim_end();
    }
...
impl<'a> SplitPair<'a> {
    ...
    pub fn new(s: &'a str, delimiter: char) -> Option<Self> {
        ...
        Some(SplitPair {
            left: &s[..pos],
            right: &s[pos + delimiter.len_utf8()..],
            delimiter,
        })
    }
    ...
    pub fn rejoin(&self) -> String {
        format!("{}{}{}", self.left, self.delimiter, self.right)
    }
...
impl fmt::Display for SplitPair<'_> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "{} '{}' {}", self.left, self.delimiter, self.right)
    }
}
...
pub fn find_all_matches<'a>(haystack: &'a str, needle: &str) -> Vec<&'a str> {
    ...
    while let Some(pos) = haystack[start..].find(needle) {
        let abs = start + pos;
        results.push(&haystack[abs..abs + needle_len]);
        start = abs + 1;
    }
    results
}
...
impl<'a> Iterator for CsvFields<'a> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        match self.remaining.find(',') {
            Some(pos) => {
                let field = self.remaining[..pos].trim();
                self.remaining = &self.remaining[pos + 1..];
                Some(field)
            }
            None => {
                self.done = true;
                Some(self.remaining.trim())
            }
        }
    }
}
...
impl<'a> Iterator for Windows<'a> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        if self.pos + self.width > self.text.len() {
            return None;
        }
        let window = &self.text[self.pos..self.pos + self.width];
        self.pos += 1;
        Some(window)
    }
}
...
pub fn interleave<'a>(a: &[&'a str], b: &[&'a str]) -> Vec<&'a str> {
    ...
    loop {
        match (ai.next(), bi.next()) {
            ...
            (Some(&x), None) => {
                result.push(x);
                result.extend(ai.copied());
                break;
            }
            (None, Some(&y)) => {
                result.push(y);
                result.extend(bi.copied());
                break;
            }
            (None, None) => break,
        }
...
impl<'text, 'note> Annotated<'text, 'note> {
    ...
    pub fn format(&self) -> String {
        format!("{} [{}@{}]", self.text, self.note, self.position)
    }
}
...
pub fn longest_matching<'a, T: AsRef<str>>(items: &'a [T], pattern: &str) -> Option<&'a T> {
    items
        .iter()
        .filter(|item| item.as_ref().contains(pattern))
...
pub fn parse_pairs<'a>(input: &'a str) -> Result<Vec<KeyValue<'a>>, String> {
    input
        ...
        .enumerate()
        .map(|(i, line)| {
            KeyValue::parse(line).ok_or_else(|| format!("invalid pair on line {}", i + 1))
        })
        .collect()
}
